...
public class ReferralTemplateResource {
  ...
  public ReferralTemplateResource(
      ReferralTemplateService referralTemplateService) {
  ...
  ReferralTemplate get(@PathVariable("id") String id) {
    return referralTemplateService.get(id).orElseThrow(
        () -> new InvalidRequestException("No such template found"));
  }
